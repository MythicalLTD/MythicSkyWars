...
public class TeamCard {
    ...
    void updateCard(int size) {
        boolean useSeparateCages = MythicSkywars.getCfg().isUseSeparateCages();
        ...
        if (player != null && ps != null && ps.isInitialized()) {
            for (PlayerCard pCard : playerCards) {
                if ((pCard.getUUID() == null) && (pCard.getSpawn() != null)) {
                    ...
                    if (MythicSkywars.getCfg().debugEnabled()) MythicSkywars.get().getLogger().info("#teamCard: pCard in reservation " + pCard.getUUID());
    ...
    public void spawnCage(PlayerCard pCard, PlayerStat ps) {
        ...
        if (cageName != null && cageName.startsWith("custom-")) {
            ...
            else Bukkit.getScheduler().runTask(MythicSkywars.get(), setCageTask); // if somehow the an event is async, ensure that it doesn't break
        }
    }
    ...
    boolean joinGame(Player player) {
        for (PlayerCard pCard : playerCards) {
            if (pCard.getUUID().equals(player.getUniqueId())) {
                ...
                Bukkit.getPluginManager().callEvent(new MythicSkywarsJoinEvent(player, gMap));
                if (MythicSkywars.getCfg().kitVotingEnabled()) {
                    gMap.getKitVoteOption().updateKitVotes();
                }
                if (MythicSkywars.getCfg().resetTimerOnJoin()) {
                    gMap.setTimer(MythicSkywars.getCfg().getWaitTimer());
                }
                return true;
            }
        }
    ...
    public boolean removePlayer(UUID uuid) {
        ...
        if ((pCard != null) /*&& (team != null)*/) {
            //team.removeEntry(MythicSkywars.get().getServer().getOfflinePlayer(uuid).getName());
            pCard.reset();
            return true;
        }
        return false;
