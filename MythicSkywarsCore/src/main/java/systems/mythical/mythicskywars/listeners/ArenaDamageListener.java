...
import systems.mythical.mythicskywars.matchevents.EnderDragonEvent;
...
public class ArenaDamageListener implements org.bukkit.event.Listener {
    ...
    public void dragonDamage(EntityDamageEvent e) {
        if (e.getEntity() instanceof EnderDragon) {
            GameMap map = MythicSkywars.getGameMapMgr().getMap(e.getEntity().getLocation().getWorld().getName());
            if (map == null) return;
    ...
    private void doProjectile(GameMap gMap, Entity damager, EntityDamageByEntityEvent event, Player victim) {
        ...
        if ((damager instanceof Snowball)) {
            event.setDamage(systems.mythical.mythicskywars.MythicSkywars.getCfg().getSnowDamage());
        }
        if ((damager instanceof Egg)) {
            event.setDamage(systems.mythical.mythicskywars.MythicSkywars.getCfg().getEggDamage());
        }
    ...
    public void onAnvilLand(EntityChangeBlockEvent event) {
        ...
        if ((event.getEntity() instanceof FallingBlock)) {
            ...
            if (systems.mythical.mythicskywars.MythicSkywars.getNMS().checkMaterial(fb, org.bukkit.Material.ANVIL)) {
                for (GameMap gMap : MythicSkywars.getGameMapMgr().getPlayableArenas(systems.mythical.mythicskywars.enums.GameType.ALL)) {
            ...
            } else if (systems.mythical.mythicskywars.MythicSkywars.getNMS().checkMaterial(fb, org.bukkit.Material.SAND)) {
                for (GameMap gMap : MythicSkywars.getGameMapMgr().getPlayableArenas(systems.mythical.mythicskywars.enums.GameType.ALL)) {
                    for (systems.mythical.mythicskywars.game.Crate crate : gMap.getCrates()) {
