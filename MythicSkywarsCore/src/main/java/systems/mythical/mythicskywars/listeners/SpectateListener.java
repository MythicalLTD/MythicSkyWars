    ...
    public void onInventoryClick(InventoryClickEvent e) {
        ...
        if (slot == 8) {
            ...
            if (item != null && !item.getType().equals(Material.AIR)) {
                ...
                if (name == null)
                    return;
                Player toSpec = MythicSkywars.get().getServer().getPlayer(name);
                if (toSpec != null && !gameMap.mapContainsDead(toSpec.getUniqueId())) {
                    player.teleport(toSpec.getLocation(), TeleportCause.END_PORTAL);
                } else {
                    MythicSkywars.get().getLogger().warning("Spectator attempted to TP to " + name + " but that player is dead or not online!");
                }
            }
        }
    }
            ...
            if (specMap != null) {
                ...
                if (arenaWorld != null && e.getTo().getWorld().equals(arenaWorld) && e.getTo().getY() < 8.0D) {
                    ...
                    if (voidRescuePending.add(id)) {
                        MythicSkywars.get().getServer().getScheduler().runTask(MythicSkywars.get(), () -> {
                            try {
