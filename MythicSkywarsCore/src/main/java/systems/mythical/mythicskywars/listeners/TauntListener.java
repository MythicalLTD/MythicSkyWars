            ...
            if (lastHandSwap.containsKey(uuid)) {
                if (System.currentTimeMillis() - ((Long)lastHandSwap.get(uuid)) < 500L) {
                    if (lastTaunt.containsKey(uuid)) {
                        if (System.currentTimeMillis() - ((Long) lastTaunt.get(uuid)) < MythicSkywars.getCfg().getCooldown() * 1000L) {
                            int cooldown = (int) ((MythicSkywars.getCfg().getCooldown() * 1000 - (System.currentTimeMillis() - ((Long) lastTaunt.get(uuid)))) / 1000L);
