...
public class ChestManager {
    ...
    public void load(Map<Integer, Inventory> itemList, String fileName) {
        itemList.clear();
        File chestFile = ChestStorageLayout.resolveDataFile(MythicSkywars.get(), fileName, true);
    ...
    private void save(Map<Integer, Inventory> chestList, ChestType ct) {
        ...
        if (chestFile.exists()) {
            try {
                ...
                MythicSkywars.get().getLogger().severe("Failed to save chestfile " + fileName + ": " + ioException.getMessage());
            }
        }
    }
                        ...
                        if (item != null && !item.getType().equals(Material.AIR)) {
                            if (chest instanceof Chest) {
                                if (random.nextInt(100) + 1 <= chance) {
                                    ...
                                    added++;
                                    if (added >= inventory.getSize() - 1 || added >= MythicSkywars.getCfg().getMaxChest()) {
                                        break adding;
                                    }
                                }
                            }
                            if (chest instanceof DoubleChest) {
                                if (random.nextInt(100) + 1 <= chance) {
                                    ...
                                    added++;
                                    if (added >= inventory.getSize() - 1 || added >= MythicSkywars.getCfg().getMaxDoubleChest()) {
                                        break adding;
                                    }
                                }
                            }
    ...
    private void fillChestGuaranteedSlots(Object chest, Inventory inventory, Map<Integer, Inventory> fill) {
        ...
        int cap = chest instanceof DoubleChest
                ? Math.min(MythicSkywars.getCfg().getMaxDoubleChest(), Math.max(1, inventory.getSize() - 1))
                : Math.min(MythicSkywars.getCfg().getMaxChest(), Math.max(1, inventory.getSize() - 1));
