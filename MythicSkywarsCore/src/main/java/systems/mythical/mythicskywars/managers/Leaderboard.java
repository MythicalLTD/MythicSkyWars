...
public class Leaderboard {
    ...
    public Leaderboard() {
        ...
        for (LeaderType type : LeaderType.values()) {
            if (MythicSkywars.getCfg().isTypeEnabled(type)) {
                ...
                if (MythicSkywars.getCfg().leaderSignsEnabled()) {
        ...
        }, 0, MythicSkywars.getCfg().getUpdateTime() * 20);
    }
    ...
    public void finishedLoading(LeaderType type) {
        ...
        topLeaders.put(type, getTop(MythicSkywars.getCfg().getLeaderSize(), type));
        loaded.put(type, true);

        // Update signs if enabled
        if (MythicSkywars.getCfg().leaderSignsEnabled() && MythicSkywars.get().isEnabled()) {
            new BukkitRunnable() {
                @Override
                public void run() {
                    updateSigns(type);
        ...
        if (MythicSkywars.get().serverLoaded() &&
                MythicSkywars.getCfg().hologramsEnabled() &&
                MythicSkywars.get().isEnabled() &&
                MythicSkywars.getHoloManager() != null) {
            new BukkitRunnable() {
                @Override
                public void run() {
                    MythicSkywars.getHoloManager().updateLeaderHolograms(type);
                }
            }.runTaskLater(MythicSkywars.get(), 1);
        }
    }
    ...
    private void getSigns(LeaderType type) {
        File leaderboardsFile = new File(MythicSkywars.get().getDataFolder(), "leaderboards.yml");

        if (!leaderboardsFile.exists()) {
            MythicSkywars.get().saveResource("leaderboards.yml", false);
        }
    ...
    private void saveSigns(LeaderType type) {
        ...
        if (leaderboardsFile.exists()) {
            ...
            try {
                ...
                MythicSkywars.get().getLogger().info("[ERROR] Failed to save leaderboards file");
            }
        }
                ...
                if (pos - 1 < top.size()) {
                    for (Location loc : signs.get(type).get(pos)) {
                        ...
                        if (sign != null) {
                            ...
                            sign.update();
                            if (MythicSkywars.getCfg().leaderHeadsEnabled()) {
    ...
    private void updateHead(Sign sign, UUID uuid) {
        ...
        if (MythicSkywars.getNMS().headCheck(h1)) {
            Skull skull = (Skull) h1.getState();
            MythicSkywars.getNMS().updateSkull(skull, uuid);
            skull.update();
        }
        if (MythicSkywars.getNMS().headCheck(h2)) {
