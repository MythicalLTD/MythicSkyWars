...
import systems.mythical.mythicskywars.events.MythicSkywarsDeathEvent;
import systems.mythical.mythicskywars.events.MythicSkywarsKillEvent;
import systems.mythical.mythicskywars.events.MythicSkywarsLeaveEvent;
...
public class PlayerManager {
    ...
    private final MythicSkywars swr;

    private final boolean debug;

    private final MatchManager matchManager;

    public PlayerManager(MythicSkywars swrIn) {
        this.swr = swrIn;
        this.debug = MythicSkywars.getCfg().debugEnabled();
    ...
    public void removePlayer(final Player playerRemoved, PlayerRemoveReason removeReason, @Nullable EntityDamageEvent.DamageCause deathCause, boolean shouldSendToLobbyIn, boolean announceToOthers) {
        ...
        MythicSkywars.getOM().removePlayer(pUuid);

        // Process extra constants
        playerData = PlayerData.getPlayerData(pUuid);
        if (playerData == null) {
            if (gameMap != null) {
                this.swr.getLogger().warning("Player data was null when removing " + playerRemoved.getName()
                        + " from map " + gameMap.getName() + " (state " + gameMap.getMatchState() + ").");
            } else if (MythicSkywars.getCfg().debugEnabled()) {
                this.swr.getLogger().info("PlayerManager::removePlayer: no PlayerData for " + playerRemoved.getName()
                        + " (normal if they were not in a match or data was already cleared).");
            }
        }
        ...
        if (gameMap != null) {
            ...
            MythicSkywars.get().getLogger().info("PlayerManager::removePlayer: " + playerRemoved.getName()
                    + " has no assigned SkyWars map (normal on quit from lobby or after cleanup).");
        }
    }
    ...
    private void removeWaitingPlayer(
        ...
        if (isWaitingForGameStart) {
            ...
            if (ps != null && !removeReason.equals(PlayerRemoveReason.DEATH)) {
                ...
                if (gameMap.getTeamSize() == 1 || MythicSkywars.getCfg().isUseSeparateCages()) {
            ...
            for (final Player p : gameMap.getAlivePlayers()) {
                Util.get().playSound(p, p.getLocation(), MythicSkywars.getCfg().getLeaveSound(), 1, 1);
            }
        } else if (isGameEnding) {
            // nothing to do right now, all general data is handled in parent method
        }
        Bukkit.getPluginManager().callEvent(new MythicSkywarsLeaveEvent(playerRemoved, gameMap));
    ...
    private boolean removePlayerInGame(
        ...
        boolean punishVoidQuit = playerRemoved.getLocation().getY() <= MythicSkywars.getCfg().getQuickDeathY();
        ...
        if (effectiveRemoveReason.equals(PlayerRemoveReason.OTHER)) {
            ...
            Bukkit.getPluginManager().callEvent(new MythicSkywarsDeathEvent(playerRemoved, effectiveDeathCause, gameMap, taggerPlayer));
            ...
            if (wasKilledByTagger) {
                ...
                Bukkit.getPluginManager().callEvent(new MythicSkywarsKillEvent(taggerPlayer, playerRemoved, gameMap));
            }
        }
        Bukkit.getPluginManager().callEvent(new MythicSkywarsLeaveEvent(playerRemoved, gameMap));
        ...
        // ---------------- GAME MAP UPDATES -----------------
        gameMap.removePlayer(pUuid);
        if (MythicSkywars.getCfg().spectateEnable() && effectiveRemoveReason.equals(PlayerRemoveReason.DEATH)) {
    ...
    public void prepareSpectateInv(Player spectatorPlayer, GameMap gameMap) {
        ...
        for (Player alivePlayer : gameMap.getAlivePlayers()) {
            if (alivePlayer != null) {
                ...
                ItemStack playerhead1 = MythicSkywars.getNMS().getBlankPlayerHead();
                SkullMeta meta1 = (SkullMeta) playerhead1.getItemMeta();
                MythicSkywars.getNMS().updateSkull(meta1, alivePlayer);
    ...
    public void updateStatsForKiller(Player killer) {
        ...
        if (killerData != null) {
            ...
            if (MythicSkywars.getCfg().isSoulWellEnabled()) {
                int per = Math.max(0, MythicSkywars.getCfg().getSoulWellSoulsPerKill());
                if (per > 0) {
                    killerData.addSouls(per * multiplier);
        ...
        if (MythicSkywars.getCfg().economyEnabled()) {
            VaultUtils.get().give(killer, multiplier * MythicSkywars.getCfg().getKillerEco());
        }
        int xpReward = LevelManager.get().getXpReward("per-kill", 10);
        ...
        Util.get().doCommands(MythicSkywars.getCfg().getKillCommands(), killer);
    }
