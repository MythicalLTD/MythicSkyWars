                        ...
                        for (Player player : gMap.getAlivePlayers()) {
                            if (player != null) {
                                ...
                                for (int i = 0; i < per5Tick; i++) {
                                    UUID uuid = MythicSkywars.getNMS().spawnFallingBlock(loc.add(ThreadLocalRandom.current().nextDouble(-3.0D, 3.0D), ThreadLocalRandom.current().nextDouble(10.0D, 20.0D), ThreadLocalRandom.current().nextDouble(-3.0D, 3.0D)), Material.ANVIL, true).getUniqueId();
