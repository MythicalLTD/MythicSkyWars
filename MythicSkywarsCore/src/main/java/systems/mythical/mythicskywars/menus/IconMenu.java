...
public class IconMenu {
    ...
    private void addPrevItem(Inventory inventory) {
        inventory.setItem(inventory.getSize() - 9, MythicSkywars.getIM().getItem("prevPageItem"));
    }

    private void addNextItem(Inventory inventory) {
        inventory.setItem(inventory.getSize() - 1, MythicSkywars.getIM().getItem("nextPageItem"));
    }

    private void addExitItem(Inventory inventory) {
        inventory.setItem(inventory.getSize() - 5, MythicSkywars.getIM().getItem("exitMenuItem"));
    }

    public void update() {
        if (MythicSkywars.get().isEnabled() && update != null) {
            org.bukkit.Bukkit.getScheduler().scheduleSyncDelayedTask(MythicSkywars.get(), update);
        }
    }
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        if ((event.getInventory().getItem(slot) != null) && (!event.getInventory().getItem(slot).getType().equals(Material.AIR))) {
            name = MythicSkywars.getNMS().getItemName(event.getCurrentItem());
        }

        if (!name.equalsIgnoreCase("uselessName")) {
            if (name.equalsIgnoreCase(MythicSkywars.getNMS().getItemName(MythicSkywars.getIM().getItem("prevPageItem")))) {
                openInventory((Player) event.getWhoClicked(), index - 1);
                return;
            }
            if ((name.equalsIgnoreCase(MythicSkywars.getNMS().getItemName(MythicSkywars.getIM().getItem("nextPageItem")))) && (index + 1 < invs.size())) {
                openInventory((Player) event.getWhoClicked(), index + 1);
                return;
            }
