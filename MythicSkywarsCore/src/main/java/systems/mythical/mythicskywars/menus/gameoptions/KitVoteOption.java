...
public class KitVoteOption {
    ...
    private void createMenu(String key, String name) {
        this.key = key;
        int menuSize = MythicSkywars.getCfg().getKitMenuSize();
        ...
        MythicSkywars.getIC().create(key, invs, event -> {
            ...
            if (kit != null) {
                ...
                Bukkit.getPluginManager().callEvent(new MythicSkywarsSelectKitEvent(event.getPlayer(), gameMap, kit));
                updateKitVotes();
                Util.get().playSound(event.getPlayer(), event.getPlayer().getLocation(), MythicSkywars.getCfg().getConfirmeSelctionSound(), 1, 1);
                event.getPlayer().closeInventory();
    ...
    private void loadKit(Player player, GameMap gMap, GameKit gameKit) {
        ...
        kit = MythicSkywars.getNMS().getItemStack(kit, lores, gameKit.getColorName());
    ...
    public void updateKitVotes() {
        ...
        for (GameKit gKit : votes.keySet()) {
            ...
            kit = MythicSkywars.getNMS().getItemStack(kit, lores, ChatColor.translateAlternateColorCodes('&', gKit.getColorName()));
