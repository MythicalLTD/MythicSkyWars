...
public class WeatherOption extends GameOption {
    ...
    public void completeOption() {
        ...
        if (weather == Vote.WEATHERTHUNDER) {
            ...
            CoordLoc center = gameMap.getApproximateIslandsCenter();
            if (mapWorld != null && center != null) {
                final int half = 64;
                    ...
                    for (int z = minZ; z <= maxZ; z++) {
                        try {
                            if (MythicSkywars.getNMS().getVersion() < 13) {
                                mapWorld.setBiome(x, z, Biome.valueOf("ICE_MOUNTAINS"));
                            } else {
                                mapWorld.setBiome(x, z, Biome.valueOf("SNOWY_TUNDRA"));
                            }
                        } catch (IllegalArgumentException ignored) {
                            // biome enum name differs on some forks
                        }
                    }
                }
                for (Chunk chunk : Util.get().getChunksInBlockRect(mapWorld, minX, maxX, minZ, maxZ)) {
                    mapWorld.refreshChunk(chunk.getX(), chunk.getZ());
                }
            }
        }
        ...
        if (MythicSkywars.getCfg().isWeatherVoteEnabled()  && gameMap.getTimer() < 5) {
            String optionValue = getVoteString(weather);
