...
public class KillSoundOption extends PlayerOption {
    ...
    private static void saveKillFile(String filename) {
        ...
        if (sf.exists() && !filename.equals("killsounds.yml")) {
            ...
            if (!result) {
                MythicSkywars.get().getLogger().info("Failed to rename Killsounds File");
            } else {
                MythicSkywars.get().getLogger().info("Migrated killsounds to cosmetics folder");
            }
        }
    }
    ...
    public static void loadPlayerOptions() {
        ...
        File oldSoundFile = new File(MythicSkywars.get().getDataFolder(), "killsounds.yml");
        if (!soundFile.exists() && oldSoundFile.exists()) {
            MythicSkywars.get().getLogger().info("Migrating killsounds.yml to cosmetics folder...");
            if (oldSoundFile.renameTo(soundFile)) {
                MythicSkywars.get().getLogger().info("Successfully migrated killsounds.yml");
            } else {
                MythicSkywars.get().getLogger().warning("Failed to migrate killsounds.yml");
            }
        }
        
        // Clean up old version-specific files if they exist
        File oldKillsounds18 = new File(MythicSkywars.get().getDataFolder(), "killsounds18.yml");
        File oldKillsounds112 = new File(MythicSkywars.get().getDataFolder(), "killsounds112.yml");
        ...
        if (!soundFile.exists()) {
            ...
            } else if (MythicSkywars.getNMS().getVersion() < 13 && MythicSkywars.getNMS().getVersion() > 8) {
                saveKillFile("killsounds112.yml");
            } else {
                MythicSkywars.get().saveResource("cosmetics/killsounds.yml", false);
            }
        }
                    ...
                    if (mat != null) {
                        if (!isCustom) {
                            try {
                                ...
                                MythicSkywars.get().getServer().getLogger().info(sound + " is not a valid sound in killsounds.yml");
                            }
                        } else {
                        ...
                        MythicSkywars.get().getServer().getLogger().info(material + " is not a valid Material in killsounds.yml");
                    }
                }
            }
        }
