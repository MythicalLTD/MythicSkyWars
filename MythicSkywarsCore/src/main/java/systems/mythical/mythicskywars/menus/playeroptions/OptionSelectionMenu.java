...
public class OptionSelectionMenu {
    ...
    public OptionSelectionMenu(final Player player, PlayerOptions p, boolean commandOpen) {
        ...
        for (PlayerOption option : availableItems) {
            ...
            ItemStack item = MythicSkywars.getIM().getItem("nopermission");
            ...
            invs.get(page).setItem(position, MythicSkywars.getNMS().getItemStack(item, loreList, ChatColor.translateAlternateColorCodes('&', option.getName())));
        }
        MythicSkywars.getIC().create(player, invs, event -> {
            ...
            if (MythicSkywars.getCfg().economyEnabled() && !player.hasPermission(option.getPermission())) {
