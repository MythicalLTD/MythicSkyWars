...
public class ParticleEffectOption extends PlayerOption {
    ...
    public static void loadPlayerOptions() {
        ...
        File particleFile = new File(cosmeticsDir, "particleeffects.yml");
        
        // Backward compatibility: migrate from old location
        File oldParticleFile = new File(MythicSkywars.get().getDataFolder(), "particleeffects.yml");
        if (!particleFile.exists() && oldParticleFile.exists()) {
            MythicSkywars.get().getLogger().info("Migrating particleeffects.yml to cosmetics folder...");
            if (oldParticleFile.renameTo(particleFile)) {
                MythicSkywars.get().getLogger().info("Successfully migrated particleeffects.yml");
            } else {
                MythicSkywars.get().getLogger().warning("Failed to migrate particleeffects.yml");
            }
        }

        if (!particleFile.exists()) {
            MythicSkywars.get().saveResource("cosmetics/particleeffects.yml", false);
        }
                ...
                for (String key : storage.getConfigurationSection("effects").getKeys(false)) {
                    ...
                    if (particles != null) {
                        for (String part : particles) {
                            ...
                            if (parts.length == 6
                                    && MythicSkywars.getNMS().isValueParticle(parts[0].toUpperCase())
                                ...
                                MythicSkywars.get().getLogger().info("The particle effect " + key + " has an invalid particle effect");
                            }
                        }
                    }
