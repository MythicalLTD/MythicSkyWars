...
public class ProjectileEffectOption extends PlayerOption {
    ...
    public static void loadPlayerOptions() {
        ...
        File oldParticleFile = new File(MythicSkywars.get().getDataFolder(), "projectileeffects.yml");
        if (!particleFile.exists() && oldParticleFile.exists()) {
            MythicSkywars.get().getLogger().info("Migrating projectileeffects.yml to cosmetics folder...");
            if (oldParticleFile.renameTo(particleFile)) {
                MythicSkywars.get().getLogger().info("Successfully migrated projectileeffects.yml");
            } else {
                MythicSkywars.get().getLogger().warning("Failed to migrate projectileeffects.yml");
            }
        }

        if (!particleFile.exists()) {
            MythicSkywars.get().saveResource("cosmetics/projectileeffects.yml", false);
        }
