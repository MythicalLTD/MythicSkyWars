...
public class TauntOption extends PlayerOption {
    ...
    private static void saveTauntFile(String filename) {
        ...
        if (sf.exists() && !filename.equals("taunts.yml")) {
            boolean result = sf.renameTo(new File(cosmeticsDir, "taunts.yml"));
            if (!result) {
                MythicSkywars.get().getLogger().info("Failed to rename Taunts File");
            } else {
                MythicSkywars.get().getLogger().info("Migrated taunts to cosmetics folder");
            }
        }
    }
    ...
    public static void loadPlayerOptions() {
        ...
        File oldTauntFile = new File(MythicSkywars.get().getDataFolder(), "taunts.yml");
        if (!tauntFile.exists() && oldTauntFile.exists()) {
            MythicSkywars.get().getLogger().info("Migrating taunts.yml to cosmetics folder...");
            if (oldTauntFile.renameTo(tauntFile)) {
                MythicSkywars.get().getLogger().info("Successfully migrated taunts.yml");
            } else {
                MythicSkywars.get().getLogger().warning("Failed to migrate taunts.yml");
            }
        }
        
        // Clean up old version-specific files if they exist
        File oldTaunts18 = new File(MythicSkywars.get().getDataFolder(), "taunts18.yml");
        File oldTaunts112 = new File(MythicSkywars.get().getDataFolder(), "taunts112.yml");
        ...
        if (!tauntFile.exists()) {
            if (MythicSkywars.getNMS().getVersion() < 9) {
                ...
                MythicSkywars.get().saveResource("cosmetics/taunts.yml", false);
            }
        }
    ...
    public void performTaunt(Player player) {
        if (!this.getKey().equalsIgnoreCase("none")) {
            MythicSkywars.getNMS().playGameSound(player.getLocation(), sound, null, volume, pitch, useCustomSound);
