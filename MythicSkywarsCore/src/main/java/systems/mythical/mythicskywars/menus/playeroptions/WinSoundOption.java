...
public class WinSoundOption extends PlayerOption {
    ...
    private static void saveWinFile(String filename) {
        MythicSkywars.get().saveResource("cosmetics/" + filename, false);
        File cosmeticsDir = new File(MythicSkywars.get().getDataFolder(), "cosmetics");
        ...
        if (sf.exists() && !filename.equals("winsounds.yml")) {
            ...
            if (!result) {
                MythicSkywars.get().getLogger().info("Failed to rename Winsounds File");
            } else {
                MythicSkywars.get().getLogger().info("Migrated winsounds to cosmetics folder");
            }
        }
    }
    ...
    public static void loadPlayerOptions() {
        ...
        File soundFile = new File(cosmeticsDir, "winsounds.yml");
        
        // Backward compatibility: migrate from old location
        File oldSoundFile = new File(MythicSkywars.get().getDataFolder(), "winsounds.yml");
        if (!soundFile.exists() && oldSoundFile.exists()) {
            MythicSkywars.get().getLogger().info("Migrating winsounds.yml to cosmetics folder...");
            if (oldSoundFile.renameTo(soundFile)) {
                MythicSkywars.get().getLogger().info("Successfully migrated winsounds.yml");
            } else {
                MythicSkywars.get().getLogger().warning("Failed to migrate winsounds.yml");
            }
        }
        
        // Clean up old version-specific files if they exist
        File oldWinsounds18 = new File(MythicSkywars.get().getDataFolder(), "winsounds18.yml");
        File oldWinsounds112 = new File(MythicSkywars.get().getDataFolder(), "winsounds112.yml");
        if (oldWinsounds18.exists()) oldWinsounds18.delete();
        if (oldWinsounds112.exists()) oldWinsounds112.delete();

        if (!soundFile.exists()) {
            if (MythicSkywars.getNMS().getVersion() < 9) {
                saveWinFile("winsounds18.yml");
            } else if (MythicSkywars.getNMS().getVersion() < 13) {
                saveWinFile("winsounds112.yml");
            } else {
                MythicSkywars.get().saveResource("cosmetics/winsounds.yml", false);
            }
        }
                    ...
                    if (mat != null) {
                        if (!isCustom) {
                            try {
                                ...
                                MythicSkywars.get().getServer().getLogger().info(sound + " is not a valid sound in winsounds.yml");
                            }
                        } else {
                        ...
                        MythicSkywars.get().getServer().getLogger().info(material + " is not a valid Material in winsounds.yml");
                    }
                }
            }
        }
    ...
    public void playSound(Location loc) {
        if (MythicSkywars.get().getConfig().getBoolean("soundsEnabled", true)) {
            MythicSkywars.getNMS().playGameSound(loc, sound, null, volume, pitch, customSound);
        }
    }
