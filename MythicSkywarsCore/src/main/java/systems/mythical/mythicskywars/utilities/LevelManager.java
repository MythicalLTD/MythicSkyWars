...
public final class LevelManager {
    ...
    public void grantLevelUpRewards(Player player, int oldXp, int newXp) {
        ...
        Bukkit.getScheduler().runTask(MythicSkywars.get(), () -> {
            Player p = MythicSkywars.get().getServer().getPlayer(uid);
            ...
            for (int level = oldLvl + 1; level <= newLvl; level++) {
                ...
                if (MythicSkywars.getMessaging().getMessage("levels.level-reward-broadcast") != null) {
