...
public class SWRServer {
    ...
    private static int rankForJoinable(MatchState s) {
    ...
    private static final Comparator<SWRServer> JOINABLE_SERVER_ORDER = new Comparator<SWRServer>() {
        @Override
        public int compare(SWRServer a, SWRServer b) {
            int ra = rankForJoinable(a.getMatchState());
            int rb = rankForJoinable(b.getMatchState());
            if (ra != rb) {
    ...
    public static SWRServer getAvailableServer() {
        ArrayList<SWRServer> candidates = new ArrayList<>();
        synchronized (serversLock) {
            for (SWRServer server : getServersCopy()) {
                if ((server.getMatchState().equals(MatchState.WAITINGSTART) || server.getMatchState().equals(MatchState.WAITINGLOBBY))
        ...
        Collections.sort(candidates, JOINABLE_SERVER_ORDER);
    ...
    private static void updateBlock(Block block, String item) {
        ItemStack itemStack = MythicSkywars.getIM().getItem(item);
    ...
    public void setMatchState(MatchState serverState) {
        Bukkit.getScheduler().callSyncMethod(MythicSkywars.get(), () -> {
            Bukkit.getPluginManager().callEvent(new MythicSkywarsMatchStateChangeEvent(this, serverState));
            return null;
        });
        synchronized (stateLock) {
    ...
    public void saveSigns() {
        ...
        MythicSkywars.get().getConfig().set("signs." + this.getServerName(), signLocs);
        MythicSkywars.get().saveConfig();
        MythicSkywars.get().reloadConfig();
        MythicSkywars.getCfg().load();
    }
        ...
        for (Location loc : getSignsCopy()) {
            ...
            if (sign != null && sign.getBlock() != null) {
                ...
                if (MythicSkywars.getCfg().debugEnabled()) {
                    MythicSkywars.get().getLogger().info("Sign at: " + b.getX() + " " + b.getY() + " " + b.getZ() + ", class: " + metaBlock.getClass());
                }
                ...
                if (attachedFace == null) {
                    if (metaSign.isWallSign()) attachedFace = BlockFace.SOUTH;
                        ...
                        MythicSkywars.get().getLogger().severe("ERROR: Sign at " + b.getX() + " " + b.getY() + " " + b.getZ() +
                                " has no attached face saved! Please replace the sign.");
                        continue;
                    }
                }
                if (MythicSkywars.getCfg().debugEnabled()) {
                    MythicSkywars.get().getLogger().info("Sign face mod " + attachedFace.getModX() + " " + attachedFace.getModY() + " " + attachedFace.getModZ());
                }
    ...
    public static void updateServerSigns() {
        for (SWRServer server : getServersCopy()) {
            server.clearSigns();
            List<String> signLocs = MythicSkywars.get().getConfig().getStringList("signs." + server.getServerName());
            if (signLocs != null) {
