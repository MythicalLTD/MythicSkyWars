...
public abstract class HologramsUtil {
    ...
    void getFC() {
        holoFile = new File(MythicSkywars.get().getDataFolder(), "holograms.yml");

        if (!holoFile.exists()) {
            MythicSkywars.get().saveResource("holograms.yml", false);
        }

        if (holoFile.exists()) {
            fc = YamlConfiguration.loadConfiguration(holoFile);
            if (MythicSkywars.getCfg().debugEnabled()) {
                MythicSkywars.get().getLogger().info(this.getClass().getName() + "#debug::getFC: fc = " + fc);
            }
            for (LeaderType type : LeaderType.values()) {
                if (fc.getConfigurationSection("leaderboard." + type.toString().toLowerCase()) != null) {
                    for (String key : fc.getConfigurationSection("leaderboard." + type.toString().toLowerCase()).getKeys(false)) {
                        ...
                        if (MythicSkywars.getCfg().debugEnabled()) {
                            MythicSkywars.get().getLogger().info(this.getClass().getName() + "#debug::getFC: key = " + key);
                        }
                    }
                }
            }
