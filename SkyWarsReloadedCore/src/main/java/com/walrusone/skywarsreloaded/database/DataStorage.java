            ...
            try {
                ...
                if (!playerFile.exists()) {
                    if (!playerFile.createNewFile()) {
                        SkyWarsReloaded.get().getLogger().warning("Could not create player file for " + pData.getId());
                        return;
                    }
                }
    ...
    private void migrateLegacyNameFileIfNeeded(File playerDataDirectory, PlayerStat pData, File uuidFile) {
        ...
        if (legacyNameFile.exists()) {
            if (!legacyNameFile.renameTo(uuidFile)) {
                SkyWarsReloaded.get().getLogger().warning("Failed to migrate legacy player file " + legacyNameFile.getName() + " -> " + uuidFile.getName());
            }
        }
    }
