...
public class GameMap {
    ...
    public boolean addPlayers(@Nullable TeamCard teamToTry, final Player player) {
        ...
        if (SkyWarsReloaded.getCfg().debugEnabled()) {
            SkyWarsReloaded.get().getLogger().info("#addPlayers: " + player.getName() + "'s PlayerStats are initialized");
            SkyWarsReloaded.get().getLogger().info("#addPlayers: MatchState: " + getMatchState().name());
        }
        ...
        if (getMatchState() == MatchState.WAITINGSTART) {
            ...
            if (teamToTry == null) { // If not in party mode
                for (TeamCard tCard : teamCards) {
                    if (SkyWarsReloaded.getCfg().debugEnabled()) {
                        SkyWarsReloaded.get().getLogger().info("#addPlayers: --teamCard: " + (tCard.getPlace() + 1));
                        SkyWarsReloaded.get().getLogger().info("#addPlayers: (" + (tCard.getPlace() + 1) + ") emptySlots: " + tCard.getEmptySlots());
                    }
