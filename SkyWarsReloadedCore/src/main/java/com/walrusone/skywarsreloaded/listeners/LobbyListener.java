...
public class LobbyListener implements org.bukkit.event.Listener {
    ...
    private void enforceLobbyEnvironment(World world) {
        if (!SkyWarsReloaded.getCfg().protectLobby() || !Util.get().isSpawnWorld(world)) {
            return;
        }
        if (SkyWarsReloaded.getCfg().isLobbyForceSunny()) {
        ...
        if (SkyWarsReloaded.getCfg().isLobbyForceDay()) {
            world.setTime(1000L);
        }
    }
    ...
    public void onLobbyWeather(WeatherChangeEvent e) {
        if (!SkyWarsReloaded.getCfg().protectLobby() || !Util.get().isSpawnWorld(e.getWorld())) {
            return;
        }
        if (SkyWarsReloaded.getCfg().isLobbyForceSunny() && e.toWeatherState()) {
