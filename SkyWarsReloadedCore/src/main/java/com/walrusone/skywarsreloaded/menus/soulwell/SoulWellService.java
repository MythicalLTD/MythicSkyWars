...
public final class SoulWellService {
    ...
    private static void beginSpin(Player player, int rolls, boolean forced) {
        ...
        int ticksPer = SkyWarsReloaded.getCfg().getSoulWellAnimTicksPerFrame();
        int frames = SkyWarsReloaded.getCfg().getSoulWellAnimFrames();
        ...
        BukkitTask task = new BukkitRunnable() {
            ...
            public void run() {
                ...
                Util.get().playSound(p, p.getLocation(), SkyWarsReloaded.getCfg().getConfirmeSelctionSound(), 0.35f, 1.2f);
                frame++;
                if (frame >= frames) {
                    SPINNING.remove(uuid);
                    applyReward(p, grant);
                    ...
                    }.runTaskLater(SkyWarsReloaded.get(), 25L);
                    cancel();
                }
            }
        }.runTaskTimer(SkyWarsReloaded.get(), 1L, ticksPer);
    ...
    private static Reward rollReward(Player player) {
        ...
        w.put(RewardKind.COINS, SkyWarsReloaded.getCfg().getSoulWellWeightCoins());
        w.put(RewardKind.XP, SkyWarsReloaded.getCfg().getSoulWellWeightXp());
        w.put(RewardKind.CAGE, SkyWarsReloaded.getCfg().getSoulWellWeightCage());
        w.put(RewardKind.KIT, SkyWarsReloaded.getCfg().getSoulWellWeightKit());
        w.put(RewardKind.PERK, SkyWarsReloaded.getCfg().getSoulWellWeightPerk());
        w.put(RewardKind.COMMAND, SkyWarsReloaded.getCfg().getSoulWellWeightCommand());
        while (true) {
            int total = 0;
            for (int v : w.values()) {
                total += Math.max(0, v);
    ...
    private static Reward randomCoins() {
        int lo = SkyWarsReloaded.getCfg().getSoulWellCoinsMin();
        int hi = SkyWarsReloaded.getCfg().getSoulWellCoinsMax();
    ...
    private static Reward buildReward(Player player, RewardKind kind) {
        switch (kind) {
            case COINS:
                return randomCoins();
            case XP:
                int xlo = SkyWarsReloaded.getCfg().getSoulWellXpMin();
                int xhi = SkyWarsReloaded.getCfg().getSoulWellXpMax();
    ...
    private static PlayerOption pickRandomGlass(Player player) {
        Set<String> allow = normalizeList(SkyWarsReloaded.getCfg().getSoulWellCageAllowList());
        Set<String> deny = normalizeList(SkyWarsReloaded.getCfg().getSoulWellCageDenyList());
    ...
    private static GameKit pickRandomKit(Player player) {
        Set<String> allow = normalizeList(SkyWarsReloaded.getCfg().getSoulWellKitAllowList());
        Set<String> deny = normalizeList(SkyWarsReloaded.getCfg().getSoulWellKitDenyList());
    ...
    private static PlayerOption pickRandomParticle(Player player) {
        Set<String> allow = normalizeList(SkyWarsReloaded.getCfg().getSoulWellPerkAllowList());
        Set<String> deny = normalizeList(SkyWarsReloaded.getCfg().getSoulWellPerkDenyList());
        List<PlayerOption> list = new ArrayList<>();
    ...
    private static String pickRandomCommandReward() {
        List<String> cmds = SkyWarsReloaded.getCfg().getSoulWellCommandRewards();
    ...
    private static final class Reward {
        ...
        private static ItemStack icon(Material m, String name) {
            ...
            return SkyWarsReloaded.getNMS().getItemStack(it, Lists.newArrayList(), ChatColor.translateAlternateColorCodes('&', name));
        }
    }
}
