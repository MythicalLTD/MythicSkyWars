...
public final class Messaging {
    ...
    private void copyDefaults(File playerFile) {
        try {
            ...
            int added = ConfigMerge.mergeMissingKeysFromResource(SkyWarsReloaded.get(), "messages.yml", playerConfig);
            if (added > 0) {
                playerConfig.save(playerFile);
                SkyWarsReloaded.get().getLogger().info("Merged " + added + " new message key(s) from messages.yml.");
            }
        } catch (IOException e) {
            SkyWarsReloaded.get().getLogger().warning("Failed to merge messages.yml defaults: " + e.getMessage());
        }
    }
