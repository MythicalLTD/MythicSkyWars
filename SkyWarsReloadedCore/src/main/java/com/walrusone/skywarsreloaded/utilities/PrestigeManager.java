...
public final class PrestigeManager {
    ...
    private static String substitutePrefixTokens(PlayerStat ps, int numericLevel, String prefix) {
        ...
        if (ps != null) {
            try {
                Player online = SkyWarsReloaded.get().getServer().getPlayer(java.util.UUID.fromString(ps.getId()));
    ...
    private String loreStatusLine(boolean selected, boolean unlocked) {
        ...
        String raw = null;
        if (SkyWarsReloaded.get() != null && SkyWarsReloaded.getMessaging() != null) {
            raw = SkyWarsReloaded.getMessaging().getMessage(key);
        }
