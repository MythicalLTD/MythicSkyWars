...
public class ASPWorldManagerImpl implements ASPWorldManager {
    ...
    public ASPWorldManagerImpl() {
        ...
        this.loader = plugin.getLoader(MythicSkywars.getCfg().getSlimeWorldManagerSource());
    ...
    public World createEmptyWorld(String name, World.Environment environment) {
        ...
        try {
            ...
            MythicSkywars.getNMS().setGameRule(bukkitWorld, "doMobSpawning", "false");
            MythicSkywars.getNMS().setGameRule(bukkitWorld, "mobGriefing", "false");
            MythicSkywars.getNMS().setGameRule(bukkitWorld, "doFireTick", "false");
            MythicSkywars.getNMS().setGameRule(bukkitWorld, "showDeathMessages", "false");
            MythicSkywars.getNMS().setGameRule(bukkitWorld, "announceAdvancements", "false");
            MythicSkywars.getNMS().setGameRule(bukkitWorld, "doDaylightCycle", "false");
    ...
    public boolean loadWorld(String worldName, World.Environment environment, boolean readOnly) {
        ...
        if (MythicSkywars.getCfg().debugEnabled()) {
            MythicSkywars.get().getLogger().info(this.getClass().getName() + "#loadWorld plugin: " + plugin);
            MythicSkywars.get().getLogger().info(this.getClass().getName() + "#loadWorld worldName: " + worldName);
        }
        ...
        if (worldData == null) {
            MythicSkywars.get().getLogger().severe("An error occurred while loading \"" + worldName + "\" from SlimeWorldManager. Does the map exist?");
            return false;
        }
        ...
        if (world != null) {
            ...
            // Unload world
            MythicSkywars.get().getServer().unloadWorld(world, false);
            if (MythicSkywars.getCfg().debugEnabled()) {
                MythicSkywars.get().getLogger().info(this.getClass().getName() + "#loadWorld unloaded world");
            }
        }

        if (MythicSkywars.getCfg().debugEnabled()) {
            MythicSkywars.get().getLogger().info(this.getClass().getName() + "#loadWorld worldData: " + worldData);
        }
    ...
    public void unloadWorld(String worldName, boolean shouldSave) {
        ...
        // Pre check
        if (world == null) {
            MythicSkywars.get().getLogger().severe("World " + worldName + " is not loaded but was attempted to be unloaded anyway!");
            return;
        }
...
//    }
    ...
    public void setWorldSettings(World world) {
        ...
        MythicSkywars.getNMS().setGameRule(world, "mobGriefing", "false");
        MythicSkywars.getNMS().setGameRule(world, "doFireTick", "false");
