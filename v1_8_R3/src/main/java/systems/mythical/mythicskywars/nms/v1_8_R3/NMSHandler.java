...
public class NMSHandler implements NMS {
    ...
    public org.bukkit.inventory.ItemStack getItemStack(org.bukkit.inventory.ItemStack item, List<String> lore, String message) {
        ...
        if (MythicSkywars.getNMS().getVersion() < 21) {
            addItemMeta.addItemFlags(ItemFlag.values());
        } else {
    ...
    public Enchantment getEnchantmentByName(String... name) {
        for (String s : name) {
            ...
            // 1.8 expects enum-style names (e.g. DURABILITY for Unbreaking); callers often pass "durability"
            enchantment = Enchantment.getByName(s.toUpperCase(Locale.ROOT));
            if (enchantment != null) {
                return enchantment;
            }
        }
